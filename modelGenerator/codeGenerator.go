...
`, g.Package.Name)
	...
	modelCodeOmnibus := fmt.Sprintf(`package %s
...
`, g.Package.Name, nilModel)

	modelCodeOmnibus = strings.ReplaceAll(modelCodeOmnibus, "{{g.Package.OdataAlias}}", g.Package.OdataAlias)
	...
	func SaveByTableName(
			...
			link string, 
			data []byte,
			) (results map[string][]map[string]any, messages []error) {
	...
	%s
		...
		for _, name := range names {
			...
			if g.OutputMode.IsSingle() {
				...
				modelCode += g.ModelStruct(set.getEntityType()) + "\n"
			...
			if g.OutputMode.IsOmnibus() {
				modelCodeOmnibus += "\n" + g.ModelStruct(set.getEntityType()) + "\n"
				modelCodeOmnibus += "\n" + g.ModelDefinition(set) + "\n"
			}
			datasets += "\n" + g.DataSet(set) + "\n"
			deleteCode += "\n" + g.DeleteCode(set) + "\n"
			...
			if generateModelStruct {
				if !modelStruct.Ignore {
					...
					result := g.ModelStruct(set.getEntityType())
					fmt.Printf("\n\n%s\n\n", result)
				}
			}
			modelCodeOmnibus += "\n" + g.ModelStruct(set.getEntityType()) + "\n"
...
`
	...
	if g.OutputMode.IsOmnibus() {
		if g.Package.Models != "" {
			code[g.Package.Models] = modelCodeOmnibus
		}
		if g.Package.Select != "" {
	...
	if len(existing) > 0 {
		...
		for _, found := range existing {
			...
			if len(different) > 0 {
				...
				// existing record is present and nothing differs
				return dereferenced, nil
			}
		}
	} else {
		// Set all fields that are selected to modified
...
func (g Generator) SaveByTableName(set edmxEntitySet) string {
...
func (g Generator) SelectByTableName(set edmxEntitySet, options string) string {
	...
	case "{{databaseName}}":
		...
		function := "SelectByTableName.{{publicName}}"

		fields := strings.Split(values.Get(SELECT), COMMA)
...
func (g *Generator) ModelStruct(entityType edmxEntityType) string {
